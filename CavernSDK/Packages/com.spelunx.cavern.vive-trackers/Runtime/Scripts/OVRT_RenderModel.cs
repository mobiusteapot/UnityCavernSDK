...
namespace Spelunx.Vive
{
    ...
    public class OVRT_RenderModel : MonoBehaviour
    {
        ...
        public sealed class RenderModelInterfaceHolder : System.IDisposable
        {
                ...
                get
                {
                    if (_instance == null && !failedLoadInterface)
                    {
                        if (Application.isEditor && Application.isPlaying == false)
                            needsShutdown = Vive_Manager.InitializeTemporarySession();

                        _instance = OpenVR.RenderModels;
                        if (_instance == null)
                        {
                            Debug.LogError("<b>[OVRT]</b> Failed to load IVRRenderModels interface version " + OpenVR.IVRRenderModels_Version);
            ...
            {
                if (needsShutdown)
                    Vive_Manager.ExitTemporarySession();
            }
        }

        private void OnModelSkinSettingsHaveChanged()
        {
        ...
        public void UpdateModel()
        {
            ...
            if (system == null || index == OVRT_TrackedObject.EIndex.None)
                return;
            ...
            if (capacity <= 1)
            {
                Debug.LogError("<b>[OVRT]</b> Failed to get render model name for tracked object " + index);
                return;
            }
        ...
        IEnumerator SetModelAsync(string newRenderModelName)
        {
            ...
            using (RenderModelInterfaceHolder holder = new RenderModelInterfaceHolder())
            {
                ...
                while (true)
                {
                    ...
                    for (int renderModelNameIndex = 0; renderModelNameIndex < renderModelNames.Length; renderModelNameIndex++)
                    {
                        ...
                        //Debug.Log("<b>[OVRT]</b> renderModels.LoadRenderModel_Async(" + renderModelNames[renderModelNameIndex] + ": " + error.ToString());
            ...
            OVRT_Events.RenderModelLoaded.Invoke(this, success);
        }
        ...
        private bool SetModel(string renderModelName)
        {
            ...
            using (var holder = new RenderModelInterfaceHolder())
            {
                if (createComponents)
                {
                    ...
                    Debug.Log("<b>[OVRT]</b> [" + gameObject.name + "] Render model does not support components, falling back to single mesh.");
                }
                ...
                if (!string.IsNullOrEmpty(renderModelName))
                {
                    ...
                    if (model == null || model.mesh == null)
                    {
                        ...
                        if (verbose)
                            Debug.Log("<b>[OVRT]</b> Loading render model " + renderModelName);
        ...
        RenderModel LoadRenderModel(CVRRenderModels renderModels, string renderModelName, string baseName)
        {
            ...
            {
                Debug.LogError(string.Format("<b>[OVRT]</b> Failed to load render model {0} - {1}", renderModelName, error.ToString()));
                return null;
            }
            ...
            if (material == null || material.mainTexture == null)
            {
                ...
                {
                    Debug.Log("<b>[OVRT]</b> Failed to load render model texture for render model " + renderModelName + ". Error: " + error.ToString());
                }
            }
        ...
        private bool LoadComponents(RenderModelInterfaceHolder holder, string renderModelName)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                {
                    if (verbose)
                        Debug.Log("<b>[OVRT]</b> Loading render model " + componentRenderModelName);
        ...
        private UnityAction<bool> _onHideRenderModelsAction;
        private UnityAction _onModelSkinSettingsHaveChanged;


        private void Awake()
        {
            _onDeviceConnectedAction += OnDeviceConnected;
            _onHideRenderModelsAction += OnHideRenderModels;
            _onModelSkinSettingsHaveChanged += OnModelSkinSettingsHaveChanged;
        }
        ...
        void OnEnable()
        {
            ...
            {
                Debug.Log("<b>[OVRT]</b> " + modelOverrideWarning);
                enabled = false;
                return;
            }
        ...
        public void UpdateComponents(CVRRenderModels renderModels)
        {
            ...
            for (int childIndex = 0; childIndex < transform.childCount; childIndex++)
            {
                ...
                system.GetControllerState((uint)index, ref controllerState, (uint)Marshal.SizeOf(typeof(VRControllerState_t)));

                var componentState = new RenderModel_ComponentState_t();

                if (!renderModels.GetComponentState(componentName, componentName, ref controllerState, ref controllerModeState, ref componentState))
                    continue;
                ...
                for (int childChildIndex = 0; childChildIndex < child.childCount; childChildIndex++)
                {
                    ...
                    if (childName == OVRT_RenderModel.k_localTransformName)
                        attach = childChild;
                }
        ...
        {
            this.index = (OVRT_TrackedObject.EIndex)newIndex;
